...
@Service
public class UserCallbackServiceImpl implements UserCallbackService {
    ...
    public BotApiMethod<?> process(CallbackQuery callbackQuery) {
        ...
        if (data.equals("available-cars")) {
            ...
            if (pageEnumElement.equals(PageEnum.CAR_COMMENT.toString())) {
                ...
                return getPenaltyPage(id, page, chatId, messageId);
            ...
            return getPickupOffice(chatId, messageId, data, user);

        } else if (user.getStep().equals(StepEnum.RETURN_OFFICE)) {

            return getReturnOffice(chatId, messageId, data, user);

        } else if (user.getStep().equals(StepEnum.PROMO_CODE)) {

            return getPromoCode(data, user, chatId, messageId);
    ...
    private SendMessage getPromoCode(String data, TelegramUser user, Long chatId, Integer messageId) {
        ...
        if (data.equals("promo-code:yes")) {
            ...
            return userTextService.checkedBooking(dto, chatId);

        }
    }

    private SendMessage getReturnOffice(Long chatId, Integer messageId, String data, TelegramUser user) {
        DeleteMessage deleteMessage = DeleteMessage.builder()
                .chatId(chatId)
                .messageId(messageId)
                .build();
    ...
    private SendMessage getPickupOffice(Long chatId, Integer messageId, String data, TelegramUser user) {
    ...
    private EditMessageText getBookingPage(int page, Long chatId, Integer messageId) {

        PageableDTO<BookingDTO> myBookings = bookingClient.getMyBookings(page, 1);

        StringBuilder message = userTextService.getUserBookings(myBookings, chatId);
    ...
    private EditMessageText getPenaltyPage(long id, int page, Long chatId, Integer messageId) {
